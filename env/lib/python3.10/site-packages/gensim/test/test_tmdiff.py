...
class TestLdaDiff(unittest.TestCase):
    ...
    def test_basic(self):
        ...
        mdiff, annotation = self.model.diff(
            self.model, n_ann_terms=self.n_ann_terms, diagonal=True
        )
    ...
    def test_identity(self):
        for dist_name in ["hellinger", "kullback_leibler", "jaccard"]:
            # test for matrix case
            mdiff, annotation = self.model.diff(
                self.model, n_ann_terms=self.n_ann_terms, distance=dist_name
            )
            ...
            self.assertTrue(
                np.allclose(np.diag(mdiff), np.zeros(mdiff.shape[0], dtype=mdiff.dtype))
            )
