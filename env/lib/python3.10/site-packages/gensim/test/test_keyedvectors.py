...
class TestKeyedVectors(unittest.TestCase):
    ...
    def test_relative_cosine_similarity(self):
        ...
        cos_sim = [
            self.vectors.similarity("good", syn)
            for syn in wordnet_syn
            if syn in self.vectors
        ]
        cos_sim = sorted(
            cos_sim, reverse=True
        )  # cosine_similarity of "good" with wordnet_syn in decreasing order
        # computing relative_cosine_similarity of two similar words
        rcs_wordnet = self.vectors.similarity("good", "nice") / sum(
            cos_sim[i] for i in range(10)
        )
    ...
    def test_most_similar_restrict_vocab(self):
        ...
        predicted = set(
            result[0]
            for result in self.vectors.most_similar("war", topn=5, restrict_vocab=5)
        )
        self.assertEqual(expected, predicted)
    ...
    def test_most_similar_with_vector_input(self):
        ...
        predicted = [
            result[0] for result in self.vectors.most_similar([input_vector], topn=5)
        ]
        self.assertEqual(expected, predicted)

    def test_most_similar_to_given(self):
        """Test most_similar_to_given returns correct results."""
        predicted = self.vectors.most_similar_to_given(
            "war", ["terrorism", "call", "waging"]
        )
    ...
    def test_similar_by_word(self):
        ...
        predicted = [
            result[0] for result in self.vectors.similar_by_word("war", topn=5)
        ]
        self.assertEqual(expected, predicted)
    ...
    def test_similar_by_vector(self):
        ...
        input_vector = self.vectors["war"]
        predicted = [
            result[0] for result in self.vectors.similar_by_vector(input_vector, topn=5)
        ]
        self.assertEqual(expected, predicted)
    ...
    def test_distance(self):
        ...
        self.assertTrue(
            np.allclose(self.vectors.distance("war", "conflict"), 0.06694602)
        )
    ...
    def test_similarity(self):
        ...
        self.assertTrue(
            np.allclose(self.vectors.similarity("war", "conflict"), 0.93305397)
        )
    ...
    def test_add_multiple(self):
        ...
        entities = [
            "___some_entity{}_not_present_in_keyed_vectors___".format(i)
            for i in range(5)
        ]
        vectors = [np.random.randn(self.vectors.vector_size) for _ in range(5)]
    ...
    def test_set_item(self):
        ...
        entities = [
            "war",
            "___some_new_entity1___",
            "___some_new_entity2___",
            "terrorism",
            "conflict",
        ]
        vectors = [
            np.random.randn(self.vectors.vector_size) for _ in range(len(entities))
        ]
    ...
    def test_load_model_and_vocab_file_replace(self):
        ...
        self.assertEqual(model.get_vecattr("ありがとう�", "count"), 123)
        self.assertEqual(model.get_vecattr("どういたしまして�", "count"), 789)
        self.assertEqual(model.key_to_index["ありがとう�"], 0)
        self.assertEqual(model.key_to_index["どういたしまして�"], 1)
        self.assertTrue(
            np.array_equal(
                model.get_vector("ありがとう�"), np.array([0.6, 0.6, 0.6], dtype=np.float32)
            )
        )
        self.assertTrue(
            np.array_equal(
                model.get_vector("どういたしまして�"),
    ...
    def test_no_header(self):
        ...
        reloadtxtkv = KeyedVectors.load_word2vec_format(
            tmpfiletxt, binary=False, no_header=True
        )
        self.assertEqual(randkv.index_to_key, reloadtxtkv.index_to_key)
        self.assertTrue((randkv.vectors == reloadtxtkv.vectors).all())
    ...
    def test_get_mean_vector(self):
        ...
        self.assertTrue(
            np.allclose(self.vectors.get_mean_vector(keys), expected_result_1)
        )
        self.assertTrue(
            np.allclose(self.vectors.get_mean_vector(keys, weights), expected_result_2)
        )
        self.assertTrue(
            np.allclose(
                self.vectors.get_mean_vector(keys, pre_normalize=False),
                expected_result_3,
            )
        )
...
class LoadWord2VecFormatTest(unittest.TestCase):
    ...
    def verify_load2vec_binary_result(self, w2v_dict, binary_chunk_size, limit):
        ...
        w2v_model = gensim.models.keyedvectors._load_word2vec_format(
        ...
        w2v_dict_postprocessed = {
            k.lstrip(): w2v_dict[k] for k in w2v_keys_postprocessed
        }
