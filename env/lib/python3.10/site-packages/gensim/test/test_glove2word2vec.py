...
class TestGlove2Word2Vec(unittest.TestCase):
    ...
    def test_conversion(self):
        ...
        try:
            self.test_model = gensim.models.KeyedVectors.load_word2vec_format(
                self.output_file
            )
            self.assertTrue(
                numpy.allclose(
                    self.test_model.n_similarity(["the", "and"], ["and", "the"]), 1.0
                )
            )
        except Exception:
            if os.path.isfile(os.path.join(self.output_file)):
                self.fail(
                    "model file %s was created but could not be loaded."
                    % self.output_file
                )
            else:
                self.fail(
                    "model file %s creation failed, check the parameters and input file format."
                    % self.output_file
                )
