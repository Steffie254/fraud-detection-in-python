...
class TestPhraseAnalysis(unittest.TestCase):
    ...
    def test_analysis_connector_words_in_between(self):
        ...
        self.assertEqual(
            result,
            ["a", "simple_sentence", "with", "many_of_the_possible", "bigrams", "with"],
        )
...
class PhrasesCommon(PhrasesData):
    ...
    def setUp(self):
        ...
        self.bigram_default = Phrases(
            self.sentences, connector_words=self.connector_words
        )
    ...
    def test_empty_inputs_on_bigram_construction(self):
        ...
        self.assertEqual(
            list(self.bigram_default[(iter(()) for i in range(2))]), [[], []]
        )
...
class TestPhrasesModel(PhrasesCommon, unittest.TestCase):
    def test_export_phrases(self):
        ...
        trigram = Phrases(
            bigram[self.sentences], min_count=1, threshold=1, delimiter=" "
        )
    ...
    def test_scoring_default(self):
        ...
        test_sentences = [["graph", "minors", "survey", "human", "interface"]]
        seen_scores = set(
            round(score, 3) for score in bigram.find_phrases(test_sentences).values()
        )
    ...
    def test_scoring_npmi(self):
        ...
        assert seen_scores == {
            0.882,  # score for graph minors
            0.714,  # score for human interface
        }

    def test_custom_scorer(self):
        """Test using a custom scoring function."""
        bigram = Phrases(
            self.sentences, min_count=1, threshold=0.001, scoring=dumb_scorer
        )
        ...
        assert (
            len(seen_scores) == 3
        )  # 'graph minors' and 'survey human' and 'interface system'
...
class TestPhrasesPersistence(PhrasesData, unittest.TestCase):
    ...
    def test_save_load(self):
        ...
        assert seen_scores == set(
            [5.167, 3.444]  # score for graph minors  # score for human interface
        )
    ...
    def test_save_load_with_connector_words(self):
        ...
        bigram = Phrases(
            self.sentences, min_count=1, threshold=1, connector_words=connector_words
        )
        with temporary_file("test.pkl") as fpath:
            bigram.save(fpath)
            bigram_loaded = Phrases.load(fpath)
...
class TestFrozenPhrasesModel(PhrasesCommon, unittest.TestCase):
    ...
    def setUp(self):
        ...
        bigram_default_phrases = Phrases(
            self.sentences, connector_words=self.connector_words
        )
...
class TestPhrasesModelCommonTerms(CommonTermsPhrasesData, TestPhrasesModel):
    ...
    def test_scoring_npmi(self):
        ...
        assert seen_scores == set(
            [0.74, 0.894]  # score for data and graph  # score for human interface
        )
