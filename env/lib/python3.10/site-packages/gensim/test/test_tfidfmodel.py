...
class TestTfidfModel(unittest.TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual(
            model1.idfs,
            tfidfmodel.precompute_idfs(model1.wglobal, dfs, len(common_corpus)),
        )
    ...
    def test_consistency(self):
        ...
        transformed_docs = [model[docs[0]], model[docs[1]]]
        expected_docs = [
            [(3, 3.0), (4, 3.0), (5, 1.0), (6, 3.0), (7, 3.0), (8, 1.8073549220576042)],
            [(5, 2.0), (9, 3.0), (10, 3.0)],
        ]
        ...
        model = tfidfmodel.TfidfModel(
            corpus, wlocal=lambda x: x, wglobal=lambda x, y: x * x, smartirs="nnc"
        )

        transformed_docs = [model[docs[0]], model[docs[1]]]

        model = tfidfmodel.TfidfModel(
            corpus, wlocal=lambda x: x * x, wglobal=lambda x, y: x, smartirs="nnc"
        )
        expected_docs = [model[docs[0]], model[docs[1]]]
        ...
        slope = 0.2
        model = tfidfmodel.TfidfModel(
            dictionary=dictionary, smartirs="nnb", slope=slope
        )
        transformed_docs = [model[docs[0]], model[docs[1]]]
        average_character_length = sum(
            len(word) + 1.0 for text in texts for word in text
        ) / len(texts)
        vector_norms = [
    ...
    def test_pivoted_normalization(self):
        ...
        expected_docs = [
            [
                (8, 0.8884910505493495),
                (7, 0.648974041227711),
                (6, 0.8884910505493495),
                (5, 0.648974041227711),
                (4, 0.8884910505493495),
                (3, 0.8884910505493495),
            ],
            [
                (10, 0.8164965809277263),
                (9, 0.8164965809277263),
                (5, 1.6329931618554525),
            ],
        ]
    ...
    def test_wlocal_wglobal(self):
        ...
        docs = [corpus[1], corpus[2]]
        model = tfidfmodel.TfidfModel(
            corpus, wlocal=wlocal, wglobal=wglobal, normalize=False
        )
        transformed_docs = [model[docs[0]], model[docs[1]]]
        expected_docs = [
