...
from gensim.scripts.segment_wiki import (
    segment_all_articles,
...
class TestSegmentWiki(unittest.TestCase):
    ...
    def test_segment_all_articles(self):
        title, sections, interlinks = next(
            segment_all_articles(self.fname, include_interlinks=True)
        )
        ...
        self.assertTrue(
            interlinks[0] == ("political philosophy", "political philosophy")
        )
    ...
    def test_segment_and_write_all_articles(self):
        tmpf = get_tmpfile("script.tst.json")
        segment_and_write_all_articles(
            self.fname, tmpf, workers=1, include_interlinks=True
        )
        ...
        title, section_titles, interlinks = (
            article["title"],
        ...
        self.assertEqual(len(interlinks), 685)
        self.assertEqual(
            tuple(interlinks[0]), ("political philosophy", "political philosophy")
        )
        self.assertEqual(tuple(interlinks[1]), ("self-governance", "self-governed"))
        self.assertEqual(
            tuple(interlinks[2]), ("stateless society", "stateless societies")
        )
...
class TestWord2Vec2Tensor(unittest.TestCase):
    ...
    def test_conversion(self):
        word2vec2tensor(
            word2vec_model_path=self.datapath, tensor_filename=self.output_folder
        )
        ...
        for word, vector in zip(metadata, vectors):
            ...
            np.testing.assert_almost_equal(
                orig_model[word_string], vector_array, decimal=5
            )
