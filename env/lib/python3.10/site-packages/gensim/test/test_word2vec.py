...
class TestWord2VecModel(unittest.TestCase):
    def test_build_vocab_from_freq(self):
        ...
        model_hs = word2vec.Word2Vec(
            vector_size=10, min_count=0, seed=42, hs=1, negative=0
        )
        model_neg = word2vec.Word2Vec(
            vector_size=10, min_count=0, seed=42, hs=0, negative=5
        )
    ...
    def test_max_final_vocab(self):
        ...
        model = word2vec.Word2Vec(
            vector_size=10, max_final_vocab=4, min_count=4, sample=0
        )
        ...
        model = word2vec.Word2Vec(
            vector_size=10, max_final_vocab=4, min_count=2, sample=0
        )
        model.scan_vocab(sentences)
        reported_values = model.prepare_vocab()
    ...
    def test_online_learning(self):
        ...
        model_hs = word2vec.Word2Vec(
            sentences, vector_size=10, min_count=0, seed=42, hs=1, negative=0
        )
        model_neg = word2vec.Word2Vec(
            sentences, vector_size=10, min_count=0, seed=42, hs=0, negative=5
        )
    ...
    def test_online_learning_from_file(self):
        ...
        vocabulary and to a trained model when using a sorted vocabulary"""
        with temporary_file(
            get_tmpfile("gensim_word2vec1.tst")
        ) as corpus_file, temporary_file(
            get_tmpfile("gensim_word2vec2.tst")
        ) as new_corpus_file:
            ...
            model_hs.train(
                ...
                total_words=model_hs.corpus_total_words,
                epochs=model_hs.epochs,
            )
    ...
    def test_sg_hs_online(self):
        ...
        model = word2vec.Word2Vec(
            sg=1, window=5, hs=1, negative=0, min_count=3, epochs=10, seed=42, workers=2
        )
    ...
    def test_rule_with_min_count(self):
        ...
        model = word2vec.Word2Vec(
            sentences + [["occurs_only_once"]], min_count=2, trim_rule=_rule
        )
    ...
    def test_persistence_word2vec_format(self):
        ...
        binary_model_kv = keyedvectors.KeyedVectors.load_word2vec_format(
            tmpf, binary=True
        )
        self.assertTrue(np.allclose(model.wv["human"], binary_model_kv["human"]))
        norm_only_model = keyedvectors.KeyedVectors.load_word2vec_format(
            tmpf, binary=True
        )
        ...
        self.assertTrue(
            np.allclose(
                model.wv.get_vector("human", norm=True), norm_only_model["human"]
            )
        )
        limited_model_kv = keyedvectors.KeyedVectors.load_word2vec_format(
            tmpf, binary=True, limit=3
        )
        ...
        self.assertEqual(
            binary_model_kv.vectors.nbytes, half_precision_model_kv.vectors.nbytes * 2
        )
    ...
    def test_too_short_binary_word2vec_format(self):
        ...
        f.close()
        self.assertRaises(
            EOFError, keyedvectors.KeyedVectors.load_word2vec_format, tfile, binary=True
        )
    ...
    def test_persistence_word2vec_format_non_binary(self):
        ...
        norm_only_model.unit_normalize_all()
        self.assertFalse(
            np.allclose(model.wv["human"], norm_only_model["human"], atol=1e-6)
        )
        self.assertTrue(
            np.allclose(
                model.wv.get_vector("human", norm=True),
    ...
    def test_persistence_word2vec_format_with_vocab(self):
        ...
        binary_model_with_vocab_kv = keyedvectors.KeyedVectors.load_word2vec_format(
            tmpf, testvocab, binary=True
        )
        self.assertEqual(
    ...
    def test_persistence_keyed_vectors_format_with_vocab(self):
        ...
        testvocab = get_tmpfile("gensim_word2vec.vocab")
        model.wv.save_word2vec_format(tmpf, testvocab, binary=True)
        kv_binary_model_with_vocab = keyedvectors.KeyedVectors.load_word2vec_format(
            tmpf, testvocab, binary=True
        )
        self.assertEqual(
            model.wv.get_vecattr("human", "count"),
    ...
    def test_vocab(self):
        ...
        self.assertEqual(
            sum(model.wv.get_vecattr(k, "count") for k in model.wv.key_to_index),
            total_words,
        )
        ...
        self.assertRaises(
            RuntimeError, word2vec.Word2Vec, corpus, min_count=total_words + 1
        )
    ...
    def test_training(self):
        ...
        model2 = word2vec.Word2Vec(
            sentences, vector_size=2, min_count=1, hs=1, negative=0
        )
    ...
    def test_locking(self):
        ...
        for sg in range(2):  # test both cbow and sg
            model = word2vec.Word2Vec(
                vector_size=4, hs=1, negative=5, min_count=1, sg=sg, window=5
            )
            model.build_vocab(corpus)
            ...
            self.assertFalse(
                (unlocked1 == model.wv.vectors[1]).all()
            )  # unlocked vector should vary
            self.assertTrue(
                (locked0 == model.wv.vectors[0]).all()
            )  # locked vector should not vary
    ...
    def test_evaluate_word_analogies(self):
        ...
        score_cosmul, sections_cosmul = model.wv.evaluate_word_analogies(
            datapath("questions-words.txt"), similarity_function="most_similar_cosmul"
        )
    ...
    def test_evaluate_word_pairs(self):
        ...
        self.assertTrue(
            0.1 < spearman < 1.0, f"spearman {spearman} not between 0.1 and 1.0"
        )
    ...
    def test_evaluate_word_pairs_from_file(self):
        ...
        with temporary_file(get_tmpfile("gensim_word2vec.tst")) as tf:
            utils.save_as_line_sentence(
                word2vec.LineSentence(datapath("head500.noblanks.cor.bz2")), tf
            )
            ...
            oov = correlation[2]
            self.assertTrue(
                0.1 < pearson < 1.0, f"pearson {pearson} not between 0.1 & 1.0"
            )
            self.assertTrue(
                0.1 < spearman < 1.0, f"spearman {spearman} not between 0.1 and 1.0"
            )
    ...
    def test_sg_hs(self):
        """Test skipgram w/ hierarchical softmax"""
        model = word2vec.Word2Vec(
            sg=1, window=4, hs=1, negative=0, min_count=5, epochs=10, workers=2
        )
    ...
    @unittest.skipIf(
        "BULK_TEST_REPS" not in os.environ,
        reason="bulk test only occasionally run locally",
    )
    def test_method_in_bulk(self):
        ...
        method_name = os.environ.get(
            "METHOD_NAME", "test_cbow_hs"
        )  # by default test that specially-troublesome one
    ...
    def test_training_cbow(self):
        ...
        # build vocab and train in one step; must be the same as above
        model2 = word2vec.Word2Vec(
            sentences, vector_size=2, min_count=1, sg=0, hs=1, negative=0
        )
        self.models_equal(model, model2)
    ...
    def test_training_sg_negative(self):
        ...
        # build vocab and train in one step; must be the same as above
        model2 = word2vec.Word2Vec(
            sentences, vector_size=2, min_count=1, sg=1, hs=0, negative=2
        )
        self.models_equal(model, model2)
    ...
    def test_training_cbow_negative(self):
        ...
        # build vocab and train in one step; must be the same as above
        model2 = word2vec.Word2Vec(
            sentences, vector_size=2, min_count=1, sg=0, hs=0, negative=2
        )
        self.models_equal(model, model2)
    ...
    def test_similarities(self):
        ...
        self.assertTrue(
            model.wv.n_similarity(["graph"], ["trees"])
            == model.wv.similarity("graph", "trees")
        )
        self.assertRaises(
            ZeroDivisionError, model.wv.n_similarity, ["graph", "trees"], []
        )
        self.assertRaises(
            ZeroDivisionError, model.wv.n_similarity, [], ["graph", "trees"]
        )
    ...
    def test_parallel(self):
        ...
        for workers in [
            ...
            model = word2vec.Word2Vec(
                corpus, vector_size=16, min_count=(10 * 33), workers=workers
            )
            origin_word = "israeli"
    ...
    def test_predict_output_word(self):
        ...
        predictions_with_neg = model_with_neg.predict_output_word(
            ["system", "human"], topn=5
        )
        self.assertTrue(len(predictions_with_neg) == 5)

        # out-of-vobaculary scenario
        predictions_out_of_vocab = model_with_neg.predict_output_word(
            ["some", "random", "words"], topn=5
        )
        ...
        kv_model_with_neg = keyedvectors.KeyedVectors.load_word2vec_format(
            tmpf, binary=True
        )
        binary_model_with_neg = word2vec.Word2Vec()
        binary_model_with_neg.wv = kv_model_with_neg
        self.assertRaises(
            RuntimeError, binary_model_with_neg.predict_output_word, ["system", "human"]
        )

        # negative sampling scheme not used
        model_without_neg = word2vec.Word2Vec(sentences, min_count=1, hs=1, negative=0)
        self.assertRaises(
            RuntimeError, model_without_neg.predict_output_word, ["system", "human"]
        )
        ...
        prediction_from_mixed = model_with_neg.predict_output_word(
            mixed_context, topn=5
        )
    ...
    def _check_old_version(self, old_version):
        ...
        model.train(
            lee_corpus_list, total_examples=model.corpus_count, epochs=model.epochs
        )
...
class TestWord2VecSentenceIterators(unittest.TestCase):
    ...
    def test_cython_line_sentence_works_with_filename(self):
        ...
        with utils.open(datapath("lee_background.cor"), "rb") as orig:
            sentences = word2vec_corpusfile.CythonLineSentence(
    ...
    def test_line_sentence_works_with_compressed_file(self):
        ...
        with utils.open(datapath("head500.noblanks.cor"), "rb") as orig:
            sentences = word2vec.LineSentence(
                bz2.BZ2File(datapath("head500.noblanks.cor.bz2"))
            )
            for words in sentences:
                self.assertEqual(words, utils.to_unicode(orig.readline()).split())
    ...
    def test_path_line_sentences(self):
        ...
        with utils.open(
            os.path.join(datapath("PathLineSentences"), "1.txt"), "rb"
        ) as orig1:
            with utils.open(
                os.path.join(datapath("PathLineSentences"), "2.txt.bz2"), "rb"
            ) as orig2:
                ...
                for words in sentences:
                    self.assertEqual(
                        words, utils.to_unicode(orig[orig_counter]).split()
                    )
                    orig_counter += 1
