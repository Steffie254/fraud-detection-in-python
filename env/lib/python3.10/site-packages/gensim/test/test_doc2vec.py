        ...
        with open(datapath("lee_background.cor")) as f:
            for i, line in enumerate(f):
                yield doc2vec.TaggedDocument(
                    utils.simple_preprocess(line), [self._tag(i)]
                )
...
sentences = [
    doc2vec.TaggedDocument(words, [i]) for i, words in enumerate(raw_sentences)
]
...
class TestDoc2VecModel(unittest.TestCase):
    ...
    def test_persistence_word2vec_format(self):
        ...
        model.save_word2vec_format(
            test_doc_word, doctag_vec=True, word_vec=True, binary=False
        )
        binary_model_dv = keyedvectors.KeyedVectors.load_word2vec_format(
            test_doc_word, binary=False
        )
        ...
        model.save_word2vec_format(
            test_doc, doctag_vec=True, word_vec=False, binary=True
        )
        binary_model_dv = keyedvectors.KeyedVectors.load_word2vec_format(
            test_doc, binary=True
        )
        self.assertEqual(len(model.dv), len(binary_model_dv))
        # test saving word embedding only
        test_word = get_tmpfile("gensim_doc2vec.w")
        model.save_word2vec_format(
            test_word, doctag_vec=False, word_vec=True, binary=True
        )
        binary_model_dv = keyedvectors.KeyedVectors.load_word2vec_format(
            test_word, binary=True
        )
    ...
    def _check_old_version(self, old_version):
        ...
        sims_to_infer = loaded_model.dv.most_similar(
            [doc0_inferred], topn=len(loaded_model.dv)
        )
        self.assertTrue(sims_to_infer)
    ...
    def test_doc2vec_train_parameters(self):
        ...
        self.assertRaises(
            TypeError, model.train, corpus_iterable=sentences, corpus_file="test"
        )
        self.assertRaises(
            TypeError, model.train, corpus_iterable=None, corpus_file=None
        )
    ...
    def test_get_offsets_and_start_doctags(self):
        ...
        (
            offsets,
            start_doctags,
        ) = doc2vec.Doc2Vec._get_offsets_and_start_doctags_for_corpusfile(tmpf, 1)
        ...
        (
            offsets,
            start_doctags,
        ) = doc2vec.Doc2Vec._get_offsets_and_start_doctags_for_corpusfile(tmpf, 2)
        ...
        (
            offsets,
            start_doctags,
        ) = doc2vec.Doc2Vec._get_offsets_and_start_doctags_for_corpusfile(tmpf, 3)
        ...
        (
            offsets,
            start_doctags,
        ) = doc2vec.Doc2Vec._get_offsets_and_start_doctags_for_corpusfile(tmpf, 4)
        ...
        (
            offsets,
            start_doctags,
        ) = doc2vec.Doc2Vec._get_offsets_and_start_doctags_for_corpusfile(tmpf, 5)
        ...
        (
            offsets,
            start_doctags,
        ) = doc2vec.Doc2Vec._get_offsets_and_start_doctags_for_corpusfile(tmpf, 6)
    ...
    def test_unicode_in_doctag(self):
        ...
        try:
            model.save_word2vec_format(
                tmpf, doctag_vec=True, word_vec=True, binary=True
            )
    ...
    def test_string_doctags(self):
        ...
        self.assertTrue(
            max(model.dv.key_to_index.values()) < len(model.dv.index_to_key)
        )
        self.assertLess(
            max(
                model.dv.get_index(str_key) for str_key in model.dv.key_to_index.keys()
            ),
            len(model.dv.vectors),
        )
        # verify dv.most_similar() returns string doctags rather than indexes
        self.assertEqual(
            model.dv.index_to_key[0], model.dv.most_similar([model.dv[0]])[0][0]
        )
    ...
    def model_sanity(self, model, keep_training=True):
        ...
        self.assertTrue(
            fire1 in sims_ids, "{0} not found in {1}".format(fire1, sims_to_infer)
        )
        ...
        self.assertTrue(
            np.allclose(list(zip(*sims))[1], list(zip(*sims2))[1])
        )  # close-enough dists

        # sim results should be in clip range if given
        clip_sims = model.dv.most_similar(
            fire1, clip_start=len(model.dv) // 2, clip_end=len(model.dv) * 2 // 3
        )
        ...
        self.assertLess(
            model.dv.similarity(fire1, alt1), model.dv.similarity(fire1, fire2)
        )
        self.assertLess(
            model.dv.similarity(fire2, alt1), model.dv.similarity(fire1, fire2)
        )
        ...
        if keep_training:
            ...
            loaded.train(
                ...
                total_examples=loaded.corpus_count,
    ...
    def test_training(self):
        ...
        model2 = doc2vec.Doc2Vec(
            corpus, vector_size=100, min_count=2, epochs=20, workers=1
        )
    ...
    def test_dbow_hs(self):
        ...
        model = doc2vec.Doc2Vec(
            list_corpus, dm=0, hs=1, negative=0, min_count=2, epochs=20
        )
        self.model_sanity(model)
    ...
    def test_dbow_hs_fromfile(self):
        ...
        with temporary_file(get_tmpfile("gensim_doc2vec.tst")) as corpus_file:
            save_lee_corpus_as_line_sentence(corpus_file)
            model = doc2vec.Doc2Vec(
                corpus_file=corpus_file, dm=0, hs=1, negative=0, min_count=2, epochs=20
            )
            self.model_sanity(model)
    ...
    def test_dbow_neg(self):
        """Test DBOW doc2vec training."""
        model = doc2vec.Doc2Vec(
            list_corpus, vector_size=16, dm=0, hs=0, negative=5, min_count=2, epochs=40
        )
        self.model_sanity(model)
    ...
    def test_mixed_tag_types(self):
        ...
        mixed_tag_corpus = [
            doc2vec.TaggedDocument(words, [i, words[0]])
            for i, words in enumerate(raw_sentences)
        ]
        model = doc2vec.Doc2Vec()
        model.build_vocab(mixed_tag_corpus)
        expected_length = len(sentences) + len(
            model.dv.key_to_index
        )  # 9 sentences, 7 unique first tokens
    ...
    def test_train_warning(self, loglines):
        ...
        model = doc2vec.Doc2Vec(
            alpha=0.025, min_alpha=0.025, min_count=1, workers=8, vector_size=5
        )
        model.build_vocab(sentences)
...
class ConcatenatedDoc2Vec:
    ...
    def infer_vector(self, document, alpha=None, min_alpha=None, epochs=None):
        return np.concatenate(
            [
                model.infer_vector(document, alpha, min_alpha, epochs)
...
def read_su_sentiment_rotten_tomatoes(dirname, lowercase=True):
    ...
    sentence_fixups = [
        (char.encode("utf-8").decode("latin1"), char) for char in chars_sst_mangled
    ]
    ...
    SentimentPhrase = namedtuple(
        "SentimentPhrase", SentimentDocument._fields + ("sentence_id",)
    )
    ...
    assert sum(1 for phrase in phrases if phrase.sentence_id is not None) == len(
        info_by_sentence
    )  # all
