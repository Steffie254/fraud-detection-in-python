...
class TestTranslationMatrix(unittest.TestCase):
    def setUp(self):
        self.source_word_vec_file = datapath(
            "EN.1-10.cbow1_wind5_hs0_neg10_size300_smpl1e-05.txt"
        )
        self.target_word_vec_file = datapath(
        ...
        self.source_word_vec = KeyedVectors.load_word2vec_format(
            self.source_word_vec_file, binary=False
        )
        self.target_word_vec = KeyedVectors.load_word2vec_format(
            self.target_word_vec_file, binary=False
        )

    def test_translation_matrix(self):
        model = translation_matrix.TranslationMatrix(
            self.source_word_vec, self.target_word_vec, self.word_pairs
        )
    ...
    def test_persistence(self):
        ...
        self.assertTrue(
            np.allclose(model.translation_matrix, loaded_model.translation_matrix)
        )
...
class TestBackMappingTranslationMatrix(unittest.TestCase):
    def setUp(self):
        ...
        self.source_doc_vec = Doc2Vec(
            documents=train_docs[:5], vector_size=8, epochs=50, seed=1
        )
        self.target_doc_vec = Doc2Vec(
            documents=train_docs, vector_size=8, epochs=50, seed=2
        )
