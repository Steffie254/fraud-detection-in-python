...
class FlsaModel:
    ...
    def __init__(
        ...
        self._word_to_index, self._index_to_word = self._create_index_dicts(
            self._vocabulary
        )
    ...
    def _check_variables(self):
        ...
        for i, doc in enumerate(self.corpus):
            if not isinstance(doc, list):
                raise TypeError("corpus variable at index ", str(i), " is not a list")
            if not len(doc) > 0:
                raise ValueError(
    ...
    def _create_sparse_global_term_weights(
        ...
        if word_weighting in ["entropy", "normal"]:
            if sparse_local_term_weights is None:
                raise ValueError(
                    "Please feed the algorithm 'sparse_local_term_weights'"
                )
    ...
    def _calculate_entropy(
        ...
        return np.array(
            [
                1 + summed_p_log_p_i / np.log2(num_documents)
                for summed_p_log_p_i in summed_p_log_p
            ]
        )
    ...
    def _calculate_idf(
        ...
        return np.array(
            [np.log2(num_documents / word_count) for word_count in summed_words]
        )
    ...
    def _calculate_probidf(
        ...
        return np.array(
            [
                np.log2((num_documents - binary_word_count) / binary_word_count)
    ...
    def _create_sparse_binary_dtm(
        ...
        for doc_index, document in enumerate(corpus):
            ...
            for word in set(document):
                binary_sparse_dtm[
                    doc_index, word_to_index[word]
                ] = binary_document_counter[word]
    ...
    def _check_passed_variables(
        ...
        if algorithm in ["flsa"]:
            ...
            if global_term_weights is None:
                raise ValueError(
                    "Please feed the method 'global_term_weights'", " to run flsa-w"
                )
        elif algorithm in [
            "flsa-e",
        ]:
            ...
            if local_term_weights is None:
                raise ValueError(
                    "Please feed the method 'local_term_weights', to run model"
                )
    ...
    def _create_probability_matrices(
        ...
        if algorithm in ["flsa"]:
            prob_document_and_topic = (
                prob_topic_given_document_transpose.T * self._prob_document_j
            ).T
            prob_document_given_topic = (
                prob_document_and_topic / prob_document_and_topic.sum(axis=0)
            )
            self._prob_word_given_document = np.asarray(
                global_term_weights / global_term_weights.sum(1)
            )
            ...
            prob_word_and_topic = (
                prob_topic_given_word_transpose.T * self._prob_word_i
            ).T
            self._prob_word_given_topic = prob_word_and_topic / prob_word_and_topic.sum(
                axis=0
            )
            if algorithm in ["flsa-w"]:
                self._prob_word_given_document = np.asarray(
                ...
                self._prob_word_given_document = np.asarray(
                    local_term_weights / local_term_weights.sum(1)
                ).T
            prob_document_given_word = (
                self._prob_word_given_document * self._prob_document_j
            ).T / np.array(self._prob_word_i)
            prob_document_given_topic = np.matmul(
                prob_document_given_word,
                self._prob_word_given_topic,
            )
            prob_topic_given_document = (
                prob_document_given_topic * self._prob_topic_k
            ).T / self._prob_document_j
            return self._prob_word_given_topic, prob_topic_given_document
    ...
    def _create_dictlist_topn(
        ...
        for topic_index in range(prob_word_given_topic.shape[1]):
            new_dict = dict()
            highest_weight_indices = prob_word_given_topic[:, topic_index].argsort()[
                -topn:
            ]
    ...
    def show_topics(
        ...
        if prob_word_given_topic.shape[0] < prob_word_given_topic.shape[1]:
            raise ValueError(
                "'prob_word_given_topic' has more columns then rows,",
                " probably you need to take the transpose.",
            )
        warning = [
            "It seems like 'prob_word_given_topic' and 'index_to_word",
        ...
        if not formatted:
            for topic_index in range(prob_word_given_topic.shape[1]):
                weight_words = ""
                sorted_highest_weight_indices = prob_word_given_topic[
                    :, topic_index
                ].argsort()[-num_words:][::-1]
                for word_index in sorted_highest_weight_indices:
                    weight_words += (
                        str(round(prob_word_given_topic[word_index, topic_index], 4))
                        + '*"'
    ...
    def get_topic_embedding(
        ...
        if method not in ["topn", "percentile"]:
            raise ValueError(
                method,
                "is not a valid option for 'method'.",
