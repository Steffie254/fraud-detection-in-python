    ...
    def load(cls, *args, **kwargs):
        ...
        if not hasattr(model, "connector_words"):
            if hasattr(model, "common_terms"):
                ...
                logger.warning(
                    "loaded older version of %s, setting connector_words to an empty set",
                    cls.__name__,
                )
        ...
        if getattr(model, "vocab", None):
            ...
            if not isinstance(word, str):
                ...
                logger.info(
                    "re-save the loaded model to avoid this upgrade in the future"
                )
                vocab = {}
                for (
                    key,
...
class Phrases(_PhrasesTransformation):
        ...
        * ``corpus_word_count`` - the total number of tokens (non-unique) in `sentences`
        ...
        if callable(scoring):
            ...
            if not missing:
                ...
                raise ValueError(
                    f"scoring function missing expected parameters {missing}"
                )
        ...
        if sentences is not None:
            ...
            self.add_lifecycle_event(
                "created", msg=f"built {self} in {time.time() - start:.2f}s"
            )
    ...
    @staticmethod
    def _learn_vocab(
        sentences, max_vocab_size, delimiter, connector_words, progress_per
    ):
    ...
    def export_phrases(self):
        ...
        for token in source_vocab:
            ...
            phrase, score = self.score_candidate(
...
class FrozenPhrases(_PhrasesTransformation):
    ...
    def __init__(self, phrases_model):
        ...
        self.add_lifecycle_event(
            "created",
            msg=f"exported {self} from {phrases_model} in {time.time() - start:.2f}s",
        )

    def __str__(self):
        return "%s<%i phrases, min_count=%s, threshold=%s>" % (
            self.__class__.__name__,
            len(self.phrasegrams),
            self.min_count,
            self.threshold,
        )
