...
if sklearn_version < parse_version("1.2"):
    ...
    def _generate_invalid_param_val_interval(interval, constraints):
        ...
        Generating an invalid value for an integer interval depends on the other
        constraints since an int is a real, meaning that it can be valid for a real
        interval. Assumes that there can be at most 2 interval constraints: one integer
        interval and/or one real interval.
