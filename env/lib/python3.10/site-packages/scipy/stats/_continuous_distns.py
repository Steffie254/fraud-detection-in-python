...
class ncx2_gen(rv_continuous):
    ...
    def _cdf(self, x, df, nc):
        cond = np.ones_like(x, dtype=bool) & (nc != 0)
        return _lazywhere(
            cond, (x, df, nc), f=_boost._ncx2_cdf, f2=lambda x, df, _: chi2._cdf(x, df)
        )
    ...
    def _sf(self, x, df, nc):
        cond = np.ones_like(x, dtype=bool) & (nc != 0)
        return _lazywhere(
            cond, (x, df, nc), f=_boost._ncx2_sf, f2=lambda x, df, _: chi2._sf(x, df)
        )
...
class truncpareto_gen(rv_continuous):
    ...
    def _entropy(self, b, c):
        return -(
            np.log(b / (1 - c**-b)) + (b + 1) * (np.log(c) / (c**b - 1) - 1 / b)
        )
